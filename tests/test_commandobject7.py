...
# TEST OVERVIEW: object instantation with a multiple word argument

test_command_1 = "git commit -m 'initial commit'"
...
def test_sysargv_set():
    ...
    # set the sys.argv
    set_sys_argv()
    assert sys.argv == ['git', 'commit', '-m', 'initial commit']
...
def test_commandobj_property_argv():
    ...
    set_sys_argv()
    c = Command()
    assert c.argv == ['commit', '-m', 'initial commit']
...
def test_commandobj_property_definitions():
    ...
    assert len(c.defs) == 1
    assert 'm' in c.defs.keys()
    assert c.defs['m'] == "initial commit"
