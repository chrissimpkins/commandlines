...
def test_sysargv_set():
    ...
    # set the sys.argv
    set_sys_argv()
    assert sys.argv == ['executable', 'subcmd', '-s', '--long', '-n', 'shortdef', '--name', 'longdef', '--nameq=longdef', 'lastpos']
...
def test_commandobj_property_argv():
    ...
    set_sys_argv()
    c = Command()
    assert c.argv == ['subcmd', '-s', '--long', '-n', 'shortdef', '--name', 'longdef', '--nameq=longdef', 'lastpos']
