...
# TEST OVERVIEW: object instantation with a long option that includes a dash as part of option name

test_command_1 = "executable --test-option lastpos"
...
def test_sysargv_set():
    ...
    # set the sys.argv
    set_sys_argv()
    assert sys.argv == ['executable', '--test-option', 'lastpos']
...
def test_commandobj_property_argv():
    ...
    set_sys_argv()
    c = Command()
    assert c.argv == ['--test-option', 'lastpos']
...
def test_commandobj_property_switches():
    ...
    assert 'test-option' in c.switches
...
def test_commandobj_property_definitions():
    ...
    assert len(c.defs) == 1
    assert 'test-option' in c.defs.keys()
    assert c.defs['test-option'] == 'lastpos'
