...
# TEST OVERVIEW: object instantation with filepaths and alternate form of long option with single dash

test_command_1 = "find . -name tests/aaa.txt"
...
def set_sys_argv():
    sys.argv = shlex.split(test_command_1)
...
def test_sysargv_set():
    ...
    # set the sys.argv
    set_sys_argv()
    assert sys.argv == ['find', '.', '-name', 'tests/aaa.txt']
...
def test_commandobj_property_mops():
    ...
    assert len(c.mops) == 4  # in this command will be read as four separate options in this property
    for test_option in ['n', 'a', 'm', 'e']:
        assert test_option in c.mops
...
def test_commandobj_property_definitions():
    ...
    assert len(c.defs) == 1
    assert 'name' in c.defs.keys()
    assert c.defs['name'] == "tests/aaa.txt"
